...
import { buildLocationLabel, formatPlanDayLabel } from './plan-dates.js'

/** Total days in a saved plan (cache-only assembly fills all 7). */
export const PLAN_DAYS = 7

/** Days generated by Claude on the main prompt; days 6–7 come from cache. */
export const CLAUDE_DAYS = 5
...
/** Filter library rows or meal objects by allergies + excludeIngredients (case-insensitive partial match). */
export function mealsMatchingProfile(cachedMeals, profileData) {
  const allergies = profileData?.allergies || []
  const excludeIngredients = profileData?.excludeIngredients || ''
  const terms = parseDenyTerms(allergies, excludeIngredients)
  if (!terms.length) return cachedMeals || []

  return (cachedMeals || []).filter((item) => {
    const ingredientNames =
      item.ingredient_names || extractIngredientNames(item.meal_json || item)
    const ingredients = (ingredientNames || []).map((i) => String(i).toLowerCase())
    return !terms.some((excl) => ingredients.some((ing) => ing.includes(excl)))
  })
}
...
export async function findMealForSlot(supabase, { context, slotTarget, excludeIds = [] }) {
  ...
  const profileFilter = {
    ...
    excludeIngredients: context.excludeIngredients,
  }

  const candidates = mealsMatchingProfile(rows || [], profileFilter).filter((row) => {
  ...
  for (let dayIndex = 0; dayIndex < PLAN_DAYS; dayIndex++) {
    ...
    for (let slotIndex = 0; slotIndex < context.slots.length; slotIndex++) {
      ...
      if (found) {
        ...
        misses.push({
          dayIndex,
          dayName: formatPlanDayLabel(dayIndex, locationLabel),
          slotIndex,
...
/**
 * Append days 6–7 (indices 5–6) from meals_library after a 5-day Claude plan.
 * Skips when mealPlan already has PLAN_DAYS days (e.g. fully cached assembly).
 */
export async function appendCacheDays(supabase, mealPlan, profileData, tdee) {
  const plan = Array.isArray(mealPlan) ? [...mealPlan] : []
  if (plan.length >= PLAN_DAYS) {
    return { mealPlan: plan, misses: [], context: buildProfileContext(profileData, tdee) }
  }
  ...
  for (const day of plan) {
    for (const m of day.meals || []) {
      if (m?._libraryId) usedIds.push(m._libraryId)
    }
  }

  for (let dayIndex = plan.length; dayIndex < PLAN_DAYS; dayIndex++) {
    const dayMeals = []
    let dayCalories = 0
  ...
  return { mealPlan: plan, misses, context }
}
...
export function buildCacheOnlyPlanMeta(profileData, context, cd) {
  ...
  return {
    ...
    planSubtitle: `7-day plan matched to ${goalLabel} — assembled from our meal library`,
    tdee: daily?.tdee,
...
export function buildGapFillPrompt(misses, profileData, context, cd, promptBlocks = {}) {
  ...
  const slots = misses
    .map(
      (m, i) =>
        `${i + 1}. ${formatPlanDayLabel(m.dayIndex, locationLabel)} ${mealTypeLabel(m.mealType)}: ~${m.slotTarget.calories} kcal, ` +
