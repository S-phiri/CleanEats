...
export function buildVarietyCuisineBlock(cuisines) {
  ...
  if (list) {
    return `VARIETY: Weight meal ideas toward these cuisine inspirations when compatible with diet and allergies: ${list}. Across the 5 days, reflect at least two different inspirations where possible; avoid three consecutive dinners in the same narrow national style unless notes say otherwise.`
  }
  return `VARIETY: Across the 5 days, vary proteins, cuisines, and cooking methods; avoid repeating the same regional style for more than two dinners in a row unless notes say otherwise.`
}
